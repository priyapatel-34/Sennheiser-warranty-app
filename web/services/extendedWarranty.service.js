...
async function activatePaidExtendedWarrantyOrder({
  session,
  shopId,
  registerId,
  pending,
  ...
  if (!order || order.displayFinancialStatus !== "PAID") {
    console.log("[EW Activation] Skipped paid-order activation — order not paid", {
      registerId,
  ...
  let matched = trustedSource;
  let planId = pending.extended_warranty_plan_id;

  if (!trustedSource) {
    for (const lineEdge of order.lineItems?.edges || []) {
      const { registerId: lineRegisterId, planId: linePlanId } =
        extractLineItemEwIds(lineEdge.node);
      if (lineRegisterId === registerId) {
        matched = true;
        if (linePlanId) planId = linePlanId;
        break;
      }
    }
  }
  ...
  if (!matched) {
    console.log("[EW Activation] Skipped paid-order activation — no line-item match", {
      registerId,
      orderId: getNumericIdFromGid(order.id),
  ...
  console.log("[EW Activation] Activating entitlement from paid order", {
    registerId,
    planId,
    orderId: getNumericIdFromGid(order.id),
    orderName: order.name,
...
export async function findPendingEntitlementsByPaidOrder(
  shopId,
  paidOrderId,
  session
) {
  const normalizedPaidOrderId = String(getNumericIdFromGid(paidOrderId) || paidOrderId);
  const [pendingRows] = await pool.query(
    `
    SELECT
      e.*,
      r.shopify_order_id AS product_shopify_order_id
    FROM extended_warranty_entitlements e
    INNER JOIN registered_products r ON r.id = e.registered_product_id
    WHERE e.shop_id = ?
      AND e.status = 'pending_payment'
      AND e.shopify_draft_order_id IS NOT NULL
    `,
    [shopId]
  );

  if (!pendingRows.length || !session) {
    return [];
  }
  ...
  for (const pending of pendingRows) {
    const draftGid = String(pending.shopify_draft_order_id).startsWith("gid://")
      ? pending.shopify_draft_order_id
      : `gid://shopify/DraftOrder/${pending.shopify_draft_order_id}`;

    try {
      const result = await admin.request(
        `
        query DraftOrderLinkedOrder($id: ID!) {
          draftOrder(id: $id) {
            order {
              id
              name
      ...
      const linkedOrder = result?.data?.draftOrder?.order;
      if (!linkedOrder?.id) continue;

      const linkedOrderId = String(getNumericIdFromGid(linkedOrder.id));
      if (linkedOrderId !== normalizedPaidOrderId) continue;

      matches.push({
        registerId: pending.registered_product_id,
        planId: pending.extended_warranty_plan_id,
        pending,
        productOrderId: pending.product_shopify_order_id,
        orderName: linkedOrder.name,
      });
    } catch (err) {
      console.warn(
        `[EW Activation] Draft order lookup failed for entitlement ${pending.id}:`,
        err.message
      );
    }
  }
...
export async function trySyncPendingEntitlementActivation({
  ...
  const entitlement = await getActiveEntitlement(shopId, registerId);
  if (!entitlement) {
    return null;
  }
  ...
  if (entitlement.status === "active") {
    ...
    if (registered?.shopify_order_id && entitlement.shopify_order_id) {
      await syncExtendedWarrantyOrderTags({
        shop: session?.shop || (await resolveShopDomain(shopId)),
        productOrderId: registered.shopify_order_id,
        purchaseOrderId: entitlement.shopify_order_id,
        session,
        registerId,
      });
    }
  ...
  if (pending.shopify_draft_order_id) {
    const draftGid = String(pending.shopify_draft_order_id).startsWith("gid://")
      ? pending.shopify_draft_order_id
      : `gid://shopify/DraftOrder/${pending.shopify_draft_order_id}`;

    const draftResponse = await admin.request(
      `
      query DraftOrderLinkedOrder($id: ID!) {
        draftOrder(id: $id) {
          order {
    ...
    const order = draftResponse?.data?.draftOrder?.order;
    const shopName = draftResponse?.data?.shop?.name;
    const activated = await activatePaidExtendedWarrantyOrder({
      session,
      shopId,
      registerId,
      pending,
  ...
  if (normalizedEmail) {
    const ordersResponse = await admin.request(
      `
      query FindPaidEwOrders($query: String!) {
    ...
    const shopName = ordersResponse?.data?.shop?.name;
    for (const edge of ordersResponse?.data?.orders?.edges || []) {
      const activated = await activatePaidExtendedWarrantyOrder({
        session,
        shopId,
        registerId,
...
//   planIndex,
// }) {
//   if (!isExtendedWarrantyOfferEnabled(ewSettings)) {
//     return { eligible: false, reason: "feature_disabled" };
//   }
...
// export function isExtendedWarrantyOfferEnabled(settings) {
//   if (!settings) return true;
//   return Boolean(Number(settings.extended_warranty_offer_enabled ?? 1));
// }
...
//     );
//     const myProductsUrl = buildProductDetailsUrl(shopDomain, registerId);
//     const shopUrl = buildShopUrl(shopDomain);

//     const emailResult = await sendShopEmail({
//       shopId,
//       templateKey: "extended_warranty_purchase",
//       to: customerEmail || registered.customer_email,
//       renderDefault: async ({ additionalNotes } = {}) => ({
//         subject: "Extended Warranty Purchase Confirmation",
...
//           currency: purchaseCurrency,
//           serialNumber: registered.serial_number,
//           activationDate: formatDisplayDate(activationDate),
//           expiryDate: formatDisplayDate(expiryDate),
//           myProductsUrl,
//           shopUrl,
//           supportUrl: shopUrl,
//           privacyUrl: buildPrivacyPolicyUrl(shopDomain),
//           termsUrl: buildTermsPolicyUrl(shopDomain),
//           additionalNotes,
//         }),
//       }),
//     });
...
//         console.warn("Extended warranty email skipped:", {
...
//         });
//       } else {
//         console.error("Extended warranty email failed:", {
//           shopId,
//           registerId,
//           to: customerEmail || registered.customer_email,
...
//       }
//     } else {
//       console.log("Extended warranty confirmation email sent", {
//         shopId,
//         registerId,
//         to: customerEmail || registered.customer_email,
