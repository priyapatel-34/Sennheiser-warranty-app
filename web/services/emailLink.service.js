...
/**
 * Builds the Shopify customer-account login URL that returns the shopper to a
 * relative storefront path after authentication completes.
 */
...
/**
 * Normalizes email-facing dates to a compact ISO date string for templates and
 * preview data without introducing locale-specific formatting.
 */
...
/**
 * Resolves a human-friendly storefront name for email copy, falling back to a
 * name derived from the shop domain when the Shopify Admin API is unavailable.
 */
...
/**
 * Detects which Shopify customer accounts flow the shop uses so email links can
 * send shoppers back to the correct login endpoint.
 */
...
/**
 * Renders the customer CTA that sends the shopper back to their product
 * details page from an email template.
 */
...
/**
 * Looks up the stored shop domain for a shop id so storefront links can be
 * generated from database-backed records.
 */
...
// /** Generic absolute URL builder for the shop's storefront, used for the fixed email footer/CTA links. */
// export function buildShopUrl(shopDomain, path = "/") {
//   const domain = normalizeShopDomain(shopDomain);
//   if (!domain) return null;
//   const normalizedPath = path.startsWith("/") ? path : `/${path}`;
...
// /** Standard Shopify storefront policy page (every store has one at this slug). */
// export function buildPrivacyPolicyUrl(shopDomain) {
//   return buildShopUrl(shopDomain, "/policies/privacy-policy");
// }

// /** Fallback Terms & Conditions link when no merchant-configured terms URL is available. */
// export function buildTermsPolicyUrl(shopDomain) {
//   return buildShopUrl(shopDomain, "/policies/terms-of-service");
// }
...
// /** Human-readable date for the fixed email templates, e.g. "15 Jan 2026". */
